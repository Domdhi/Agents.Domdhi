/**
 * Haiku Runner — shared `claude -p` invocation + envelope parsing.
 *
 * Replaces three near-verbatim implementations:
 *   - memory-curator.js:45-111   (checkClaudeCli, invokeHaiku, parseHaikuResult, tryParseInnerJson, extractTokenCounts)
 *   - memory-benchmark.js:52-233 (identical shape, pipeline-specific payload fallback was dropped)
 *   - memory-extractor.js:29-143 (inline, raw-payload parsing — uses tryParseInnerJson primitive)
 *
 * Consumers differ on two axes:
 *   - Timeout: curator/benchmark use 90s; extractor uses 30s → configurable via opts.timeout.
 *   - Envelope: curator/benchmark expect `{result, usage}` envelope → parseHaikuResult.
 ...
 * @param {string} [opts.model]       Model flag (default: claude-haiku-4-5)
 * @param {string} [opts.logTag]      Prefix for stderr warnings on failure; pass null for silent
 * @returns {string|null}
 */
function invokeHaiku(prompt, { cwd, timeout = DEFAULT_TIMEOUT_MS, model = DEFAULT_MODEL, logTag = 'haiku-runner' } = {}) {
    ...
    try {
        ...
        if (logTag) {
            ...
            process.stderr.write(`[${logTag}] Haiku invocation failed: ${errMsg}\n`);
        }
        return null;
    }
}
