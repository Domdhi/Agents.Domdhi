// AC→source map (Task #3 / haiku-runner):
//   Five exports from _lib/haiku-runner.js:
//     - checkClaudeCli()                  — boolean probe
//     - invokeHaiku(prompt, opts)         — returns raw stdout string or null
//     - parseHaikuResult(raw)             — envelope-aware, extracts inner JSON
...
const {
    ...
    tryParseInnerJson,
    extractTokenCounts,
} = require('../haiku-runner');

// Note: `invokeHaiku` + `checkClaudeCli` hit the real subprocess by design.
...
describe('parseHaikuResult', () => {
  ...
  it('parseHaikuResult_nullInput_returnsNull', () => {
    expect(parseHaikuResult(null)).toBeNull();
    expect(parseHaikuResult('')).toBeNull();
  });

  it('parseHaikuResult_resultEnvelope_extractsInnerJson', () => {
    ...
    expect(parseHaikuResult(envelope)).toEqual(inner);
  });

  it('parseHaikuResult_textEnvelope_extractsInnerJson', () => {
    const inner = { learnings: [{ title: 'x' }] };
    const envelope = JSON.stringify({ text: JSON.stringify(inner) });
    expect(parseHaikuResult(envelope)).toEqual(inner);
  });

  it('parseHaikuResult_outputEnvelope_extractsInnerJson', () => {
    const inner = { kind: 'alt' };
    const envelope = JSON.stringify({ output: JSON.stringify(inner) });
    expect(parseHaikuResult(envelope)).toEqual(inner);
  });

  it('parseHaikuResult_contentEnvelope_concatenatesAndParses', () => {
  ...
  it('parseHaikuResult_markdownFencedInner_stripped', () => {
  ...
  it('parseHaikuResult_nonJsonInputWithFences_stripsAndParses', () => {
    ...
    const raw = '```json\n' + JSON.stringify(inner) + '\n```';
    expect(parseHaikuResult(raw)).toEqual(inner);
  });

  it('parseHaikuResult_envelopeWithoutKnownField_returnsNull', () => {
    ...
    expect(parseHaikuResult(JSON.stringify({ some_unknown_field: 'foo' }))).toBeNull();
    expect(parseHaikuResult(JSON.stringify([1, 2, 3]))).toBeNull();
  });
