...
description: "Structured review playbook with intake triage, risk-based routing, fast-lane and deep review checklists. Triggers: code review, review playbook, risk routing, review depth"
metadata:
  version: 1.0.0
  author: Domdhi.Agents
  tags: [code-review, playbook, risk-routing, triage, severity]
...
Structured review workflow that routes review depth based on change type and directory risk. Use alongside the `code-reviewer` skill — this playbook provides the routing logic while the reviewer skill provides the report template and checklists.
...
Consistent with the `code-reviewer` skill definitions:
...
The playbook routes the review depth, but findings are reported using the `code-reviewer` skill's report template. The playbook adds one section to the report:
...
- **Report template**: `.claude/skills/code-reviewer/SKILL.md`
- **Risk map source**: `code-reviewer` agent's `## Project Context > ### Risk Map` (generated by `/specialize`)
